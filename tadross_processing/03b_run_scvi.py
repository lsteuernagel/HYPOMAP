...
data_filepath_full = parameter_dict["merged_file_name"]+".h5ad"
feature_set_file = parameter_dict["feature_set_file"]
results_path = parameter_dict["project_path"]
new_name_suffix = parameter_dict["project_name"]
...
if not categorical_covariates == None:
...
if not continuous_covariates == None:
...
# check batch_var
if not batch_var in adata_scvi.obs.columns.values.tolist():
    print("batch_var is not a column name of adata.obs. Will use None for scvi.")
    batch_var = None
